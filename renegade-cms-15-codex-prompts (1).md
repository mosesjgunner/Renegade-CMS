...
`platform foundation → publications and canonical content → flagship publishing → public presentation → visual composition → identity and safety → extension contracts → AI → media → social command center → audience/email → sovereign finance and commerce → analytics → portability and production launch`
...
4. Contracts and stored data are versioned. Prefer additive evolution, preserve unknown fields/blocks, require migrations for breaking changes, test old-core/new-module and new-core/old-module boundaries, and block unsafe downgrades. “Compatible” means proven by a declared range and fixtures, not assumed.
...
12. Run the repository’s actual formatting, lint, type-check, test, migration, and build commands. Record the exact results and any pre-existing failures.
...
Milestone 1 performs the one-time repository and research inventory. Milestones 2–15 use that index and read only relevant material. There is no separate pseudocode agent or planning pass. When a risky contract genuinely needs explanation, record a concise ADR and continue implementing in the same run.
...
- An editor can configure a brand, create the main publication plus a member-style sub-publication, create an author, build Section → Category → Subcategory organization, add a reusable source and media asset, and create minimal content scoped correctly to each publication.
...
This milestone proves Renegade CMS’s first signature promise: excellent article, op-ed, essay, research-brief, press-release, and quick-post publishing with serious sourcing and human-controlled workflow.
...
4. Implement drafts, autosave, preview, immutable publication/revision history, revision comparison, restoration as a new revision, and an auditable workflow: draft → review → approved → scheduled → published → updated → archived/rejected. Enforce role transitions and separation where configured.
...
6. Implement forum-specific technical SEO and index policy: one stable canonical thread identity across pagination, permanent post anchors, correct author/date/reply markup, qualified UGC links, segmented sitemap inclusion, public/private boundaries, merge/move redirects, and explicit index/hold/noindex states for useful, new/unreviewed, thin, duplicate, private, removed or spam content. Do not mark a thread “updated” as editorially refreshed merely because it received a trivial reply.
...
- Core routes pass the project’s automated accessibility and performance thresholds.
...
Gate 7A — Passwordless identity and recovery:
...
Gate 7B — Profiles, Spaces, relationships, blogs, albums, and portfolios:
...
Gate 7C — Lightweight native private messaging:
...
Gate 7D — Publications, forums, and comments:
...
Gate 7E — Trust, moderation, abuse prevention, and application security:
...
- Two Members exchange normal and encrypted messages, verify fingerprints, recover the sender’s encrypted history from an authorized recovery copy, detect tampering, burn one encrypted message through key destruction/purge, survive worker/realtime failure, block further contact, and report only explicitly selected plaintext without granting moderators general conversation access. Database, logs, notifications and backups contain no encrypted-message plaintext.
- A Member can create or be invited to a Publication only under configured policy, cannot access another Publication’s drafts/settings, and can be suspended without deleting their identity or content history.
...
```text
Continue from the content, media, jobs, providers, and AI foundations. Read AGENTS.md, PROJECT_STATE.md, mapped social API research, and each target network’s documented capability constraints already stored in the project. Implement this milestone directly and stop at its acceptance gate.

This milestone proves “create once, distribute everywhere” while respecting that social networks have unequal and changing APIs.
...
- A private Space planning entry, a public event and a campaign launch appear in the correct scoped views; direct access, ICS feed and notifications respect each entry’s visibility and owner.
...
- A permitted Member enables commerce on their Space, connects an available processor or external-payment-link adapter plus a Dogecoin/crypto fixture, accepts a direct donation in the configured local currency, and cannot access another Space’s credentials, customers or financial history. Switching merchant/buyer country or currency recomputes eligible methods without source-code changes. Disabling crypto removes its UI/network work but preserves authorized records; re-enabling restores it.
...
This milestone connects content → distribution → audience → conversion/revenue without turning analytics into surveillance or presenting incomparable provider metrics as equivalent.
...
5. Implement bounded attribution models beginning with transparent last-non-direct and first-touch views. Preserve path examples such as social → article → signup and podcast → article → affiliate click. State uncertainty; do not infer external conversions without provider evidence.
...
- A test journey from social-tagged link → article → newsletter signup appears once in the correct funnel and attribution views.
- A test journey from campaign → support page → completed contribution appears once with correct gross amount, fee/net provenance, currency and provider attribution.
...
This distinction prevents “plug-and-play” from becoming a collection of buttons that do not work.

## Immediate execution instruction

Start or resume only Milestone 1. If work already began or was interrupted, inspect and preserve it; do not delete, reset or regenerate the repository. Reconcile the existing foundation with the settled optional-capability/compatibility constitution, passwordless identity, autonomous Member Spaces, retention/burn, optional public-key message encryption, commerce ownership, updates and managed-hosting isolation contracts, then finish the first incomplete proof gate. After Milestone 1 passes, preserve `PROJECT_STATE.md`, decisions and verification evidence. Run later work with the one-line continuation above so each run reads repository state rather than reconstructing the project from this document.
