...
import {
  executeQualityScan,
  ignoreQualityIssue,
  markStaleQualityScans,
  queueQualityScan,
} from '../../src/modules/quality/service'
  ...
  it('publishes ordered chapters, preserves a chapter redirect, and retains deterministic quality lifecycle state', async () => {
    ...
    const book = (await payload.create({
      ...
      data: {
        ...
        title: 'Quality proof',
        description: 'A complete metadata description.',
        slug: `quality-proof-${suffix}`,
        canonicalPath: `/books/quality-proof-${suffix}`,
        status: 'published',
        publishedAt: new Date().toISOString(),
        seoCanonicalURL: 'invalid',
        visibility: 'public',
      },
      overrideAccess: true,
    } as never)) as any
    const chapter = (await payload.create({
      ...
      data: {
        ...
        title: 'One',
        slug: `one-${suffix}`,
        canonicalPath: `/books/quality-proof-${suffix}/one-${suffix}`,
        displayOrder: 1,
        status: 'published',
        publishedAt: new Date().toISOString(),
        footnotes: [{ id: 'f1', text: 'A supported footnote.' }],
      },
      overrideAccess: true,
    } as never)) as any
    ...
    await payload.update({
      ...
      id: chapter.id,
      data: {
        canonicalPath: `/books/quality-proof-${suffix}/renamed-${suffix}`,
        slug: `renamed-${suffix}`,
      },
      overrideAccess: true,
    } as never)
    const redirect = await payload.find({
      ...
      where: { and: [{ site: { equals: site.id } }, { fromPath: { equals: oldPath } }] },
      limit: 1,
      depth: 0,
      overrideAccess: true,
    } as never)
    expect(redirect.docs[0]).toMatchObject({
      toPath: `/books/quality-proof-${suffix}/renamed-${suffix}`,
      statusCode: '308',
    })

    const scan = (await payload.create({
      collection: 'quality-scans',
      data: { site: site.id, targetType: 'book', targetId: book.id, status: 'queued' },
      overrideAccess: true,
    } as never)) as any
    expect(await executeQualityScan(payload, { scanId: scan.id })).toMatchObject({
      created: 1,
      findings: 1,
    })
    ...
    const rescan = (await payload.create({
      collection: 'quality-scans',
      data: { site: site.id, targetType: 'book', targetId: book.id, status: 'queued' },
      overrideAccess: true,
    } as never)) as any
    ...
    const advisory = (await payload.create({
      collection: 'quality-issues',
      data: {
        site: site.id,
        dedupeKey: `manual-${suffix}`,
        targetType: 'book',
        targetId: book.id,
        severity: 'warning',
        status: 'open',
    ...
    expect(
      await ignoreQualityIssue(payload, {
        issueId: advisory.id,
        actorRole: 'owner',
    ...
    expect(
      await markStaleQualityScans(payload, { olderThan: new Date('2026-01-01T00:00:00.000Z') }),
    ).toBeGreaterThan(0)
    expect(
