...
function gatedPayload(state: 'pending_request' | 'declined') {
  const query = vi.fn(async (text: string) => {
    ...
    if (text.includes('FOR SHARE'))
      return { rows: [{ request_state: state, request_recipient_member_id: recipient }] }
    return { rows: [] }
  })
  return {
...
describe('COMM-07B message request gates', () => {
  ...
  it('does not allow a pending recipient reply and suppresses a declined request', async () => {
    await expect(
      sendConversationMessage(gatedPayload('pending_request'), {
        siteId: site,
        conversationId: 'c',
        senderId: recipient,
        body: 'reply',
    ...
    await expect(
      sendConversationMessage(gatedPayload('declined'), {
        siteId: site,
        conversationId: 'c',
        senderId: sender,
        body: 'again',
