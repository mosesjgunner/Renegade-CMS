...
const ids = {
  ...
  case: '00000000-0000-7000-8000-000000000002',
  actor: '00000000-0000-7000-8000-000000000003',
  comment: '00000000-0000-7000-8000-000000000004',
  author: '00000000-0000-7000-8000-000000000005',
}
...
describe('COMM-05B moderation actions, sanctions, quarantine, and outbox', () => {
  ...
  it('quarantines a comment, audits the action and commits moderation.action.v1 in the same transaction', async () => {
    ...
    const query = async (text: string) => {
      ...
      if (text.includes('FROM comments'))
        return { rows: [{ authorId: ids.author, threadId: 'thread-1' }] }
  ...
  it('rejects active posting sanctions and permits an expired five-second suspension', async () => {
    const activePayload = {
      db: { pool: { query: async () => ({ rows: [{ id: 'sanction-1' }] }) } },
    } as never
    await expect(
      assertMemberCanPost(activePayload, { siteId: ids.site, memberId: ids.author }),
    ).rejects.toMatchObject({
      status: 403,
      code: 'MEMBER_POSTING_SUSPENDED',
    } satisfies Partial<ModerationActionError>)
    const expiredPayload = { db: { pool: { query: async () => ({ rows: [] }) } } } as never
    await expect(
      assertMemberCanPost(expiredPayload, { siteId: ids.site, memberId: ids.author }),
