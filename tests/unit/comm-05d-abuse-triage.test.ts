...
import { batchDispositionModerationCases, evaluateAbuseHeuristics, listModerationCases, simHash, simHashDistance, triageSubmission } from '@/modules/community/abuse-triage'
...
describe('COMM-05D abuse triage', () => {
  ...
  it('marks an unverified image attachment preview for blur in the console response', async () => {
    const payload = {
      db: { pool: { connect: async () => ({
        query: async () => ({ rows: [{ id: 'case-1', rule_categories: [], triage_payload: { sensitiveMedia: true } }] }),
        release: () => undefined,
      }) } },
    }
  ...
  it('returns itemized partial success for mixed valid and invalid batch IDs', async () => {
    ...
    const payload = {
      db: { pool: { connect: async () => ({
        query: async (text: string, values?: unknown[]) => {
          calls.push(text)
          if (text.includes('SELECT id FROM moderation_cases')) return { rows: values?.[0] === 'valid' ? [{ id: 'valid' }] : [] }
          return { rows: [] }
        },
        release: () => undefined,
      }) } },
    ...
    const items = await batchDispositionModerationCases(payload as never, {
      siteId: 'site', actorMemberId: 'moderator', caseIds: ['valid', 'missing'], disposition: 'close',
    })
    expect(items).toEqual([{ caseId: 'valid', success: true }, { caseId: 'missing', success: false, code: 'CASE_NOT_FOUND' }])
