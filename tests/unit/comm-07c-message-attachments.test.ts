...
import {
  cleanupOrphanedMessageAttachments,
  messageAttachmentMaxBytes,
  scanMessageAttachment,
  linkMessageAttachments,
} from '@/modules/community/message-attachments'
...
describe('COMM-07C governed message attachments', () => {
  it('persists restricted statuses, ten megabyte boundary, and orphan cleanup index', async () => {
    ...
    expect(schema).toContain('message_attachments')
    expect(schema).toContain('pending_scan')
    expect(schema).toContain('quarantined')
    expect(schema).toContain(String(messageAttachmentMaxBytes))
    expect(schema).toContain('orphan_cleanup_idx')
  })
  it('mints a direct PUT capability that expires exactly after fifteen minutes', () => {
    ...
    expect(signed.expiresAt).toBe('2026-09-22T12:15:00.000Z')
  ...
  it('rejects ELF bytes renamed as PNG during worker scan', async () => {
    ...
    const config = {
      storage: {
        driver: 'local',
        mediaDir: '/tmp/comm-07c-no-object',
        maxUploadBytes: messageAttachmentMaxBytes,
      },
    } as never
    await expect(scanMessageAttachment(payload, config, 'a')).resolves.toBe('rejected')
    expect(query).toHaveBeenCalledWith(expect.stringContaining('UPDATE message_attachments'), [
      'a',
      'rejected',
    ])
  })
  it('refuses cross-site attachment linking', async () => {
    ...
    await expect(
      linkMessageAttachments(query, {
        siteId: 'site-a',
        memberId: 'member-a',
        messageId: 'message-a',
        attachmentIds: ['00000000-0000-7000-8000-000000000001'],
      }),
    ).rejects.toMatchObject({ code: 'ATTACHMENT_SCOPE_DENIED', status: 403 })
  })
  it('deletes unlinked expired metadata and its storage object', async () => {
    ...
    try {
      ...
      const count = await cleanupOrphanedMessageAttachments(
        { db: { pool: { query } } } as never,
        {
          storage: {
            driver: 's3',
            s3,
