...
describe('COMM-05A moderation policy registry and report evidence', () => {
  ...
  it('keeps a report snapshot hash stable after the live target changes or is deleted', () => {
    const reported = {
      version: 1,
      targetType: 'comment',
      target: { id: 'c1', body: 'reported text', status: 'visible' },
    }
    ...
    expect(
      snapshotHash({ version: 1, targetType: 'comment', target: liveTargetAfterEdit }),
    ).not.toBe(storedHash)
  })
  ...
  it('coalesces fifteen parallel reports into exactly one parent case', async () => {
    ...
    const query = async (text: string) => {
      ...
      if (text.includes('FROM comments'))
        return { rows: [{ id: '00000000-0000-7000-8000-000000000003', body: 'reported' }] }
      if (text.includes('pg_advisory_xact_lock')) {
        if (state.locked) await new Promise<void>((resolve) => state.waiters.push(resolve))
        state.locked = true
        return { rows: [] }
      }
      if (text.includes('FROM moderation_cases'))
        return { rows: state.caseId ? [{ id: state.caseId }] : [] }
      if (text.includes('INSERT INTO moderation_cases')) {
        state.caseId = '00000000-0000-7000-8000-000000000010'
        return { rows: [{ id: state.caseId }] }
      }
      if (text.includes('INSERT INTO community_reports'))
        return { rows: [{ id: `report-${++state.reports}` }] }
      return { rows: [] }
    }
    const payload = {
    ...
    const reports = await Promise.all(
      Array.from({ length: 15 }, (_, index) =>
        ingestModerationReport(payload, {
          siteId: '00000000-0000-7000-8000-000000000001',
          reporterId: `00000000-0000-7000-8000-0000000000${String(index + 20).padStart(2, '0')}`,
          targetType: 'comment',
    ...
    expect(new Set(reports.map((report) => report.caseId))).toEqual(
      new Set(['00000000-0000-7000-8000-000000000010']),
    )
    expect(state.reports).toBe(15)
  })

  it('never serializes reporter identity into author/public output', () => {
    const output = serializeReportForAuthor({
      id: 'r1',
      site_id: 's1',
      reporter_id: 'secret-member',
      target_type: 'comment',
      target_id: 'c1',
      reason: 'abuse',
      status: 'pending',
      target_snapshot_payload: { authorId: 'other' },
    })
