...
function payloadFor(target: Record<string, unknown> = {}) {
  ...
  const query = vi.fn(async (text: string) => {
    if (text.includes('FROM comments c JOIN comment_threads')) {
      return { rows: [{ id: commentId, status: 'visible', siteId, threadFrozen: false, threadClosed: false, ...target }] }
    }
  ...
  return {
    ...
    db: { pool: { query } },
    query,
  } as never as { findByID: ReturnType<typeof vi.fn>; db: { pool: { query: typeof query } }; query: typeof query }
}
...
describe('COMM-03C comment reactions', () => {
  ...
  it('makes 50 concurrent toggles for one member converge to the deterministic absent state', async () => {
    ...
    const results = await Promise.all(Array.from({ length: 50 }, () => toggleCommentReaction(payload as never, { siteId, commentId, memberId, reactionCode: 'thumbs_up' })))
    expect(results.at(-1)).toMatchObject({ added: false, count: 0 })
    const mutationSql = payload.query.mock.calls.find(([text]) => String(text).includes('pg_advisory_xact_lock'))?.[0]
    expect(String(mutationSql)).toContain('ON CONFLICT (comment_id, member_id, reaction_code) DO NOTHING')
  })
  ...
  it('rebuilds deliberately corrupted counters solely from raw reactions', async () => {
    ...
    expect(String(query.mock.calls[0]?.[0])).toContain('DELETE FROM comment_reaction_counters WHERE comment_id = $1')
    expect(String(query.mock.calls[0]?.[0])).toContain('FROM comment_reactions WHERE comment_id = $1')
  ...
  it.each([
    ...
    await expect(toggleCommentReaction(payload as never, { commentId, memberId, ...input })).rejects.toBeInstanceOf(CommentReactionError)
  })
})
