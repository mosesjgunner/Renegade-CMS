...
import {
  ...
  listForumTopics,
  markForumTopicRead,
} from '@/modules/community/forum-browsing'
...
function payload(posts = 0) {
  const query = vi.fn(async (text: string, values: unknown[] = []) => {
    if (text.includes('FROM forum_topics WHERE id'))
      return { rows: [{ id: topicId, spaceId, lastPostSequenceNumber: posts }] }
    ...
    if (text.includes('FROM forum_posts WHERE topic_id')) {
      ...
      return {
        rows: Array.from({ length: Math.min(limit, Math.max(0, posts - after)) }, (_, index) => ({
          id: `post-${after + index + 1}`,
          sequenceNumber: after + index + 1,
        })),
      }
    }
    if (text.includes('member_topic_read_state'))
      return { rows: [{ memberId, topicId, lastReadSequenceNumber: posts }] }
    if (text.includes('FROM forum_topics t JOIN'))
      return { rows: [{ id: topicId, unread: true, lastPostSequenceNumber: posts }] }
    return { rows: [] }
  })
  return { db: { pool: { query } }, query } as unknown as Payload & { query: typeof query }
}
...
describe('COMM-04C: forum browsing, summaries, and unread cursors', () => {
  ...
  it('pages a 10,000-post topic by sequence cursor with no omissions, duplicates, or OFFSET', async () => {
    const p = payload(10_000)
    let cursor: string | null = null
    const seen: number[] = []
    do {
      const page = await getForumTopicPosts(p, {
        siteId,
        topicId,
        memberId,
        cursor,
    ...
    expect(p.query.mock.calls.filter(([sql]) => /OFFSET/i.test(sql))).toHaveLength(0)
  })
  ...
  it('exposes sort modes and only allows bounded, accessible topic listing', async () => {
    ...
    for (const sort of ['latest_activity', 'creation_date', 'most_replies', 'unanswered'])
      await expect(
        listForumTopics(p, { siteId, memberId, sort, limit: 1_000 }),
      ).resolves.toHaveLength(1)
    expect(
      p.query.mock.calls.find(([sql]) => sql.includes('FROM forum_topics t JOIN'))?.[0],
    ).toContain('space_memberships')
  })

  it('marks a topic read at its current sequence; a later sequence is unread again', async () => {
    const p = payload(12)
    await expect(markForumTopicRead(p, { siteId, topicId, memberId })).resolves.toMatchObject({
      lastReadSequenceNumber: 12,
    })
    ...
    const statement = p.query.mock.calls.find(([sql]) =>
      sql.includes('FROM forum_topics t JOIN'),
    )?.[0]
    expect(statement).toContain(
      'last_post_sequence_number > COALESCE(rs.last_read_sequence_number, 0)',
    )
  })
})
