...
import { resolveCommunityActor, CommunityError, enforceCommunityWriteRateLimit } from '@/modules/community/service'
import { ingestModerationReport, moderationTargetTypes } from '@/modules/community/moderation-reports'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await ingestModerationReport(
      payload,
      {
        siteId,
    ...
    return Response.json({ success: true, reportId: result.reportId, caseId: result.caseId }, { status: 201 })
  } catch (err: unknown) {
