...
export async function POST(
  ...
  if (!staff(auth.user)) {
    return NextResponse.json({ error: 'Legacy migration activation requires staff access.' }, { status: 403 })
  }
  ...
  try {
    const report = await activateLegacyMigration(runId, store, String(auth.user?.id ?? 'staff'))
