...
export async function POST(request: Request, context: { params: Promise<{ site_id: string; topic_id: string }> }) {
  ...
  if (!actor.memberId) return json({ error: { code: 'UNAUTHORIZED', message: 'Authentication required to record topic reads.' } }, 401)
  try {
    const data = await markForumTopicRead(payload, { siteId: site_id, topicId: topic_id, memberId: actor.memberId })
    return json({ data })
  } catch (error) {
