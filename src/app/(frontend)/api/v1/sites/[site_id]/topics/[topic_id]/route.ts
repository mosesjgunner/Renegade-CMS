...
import {
  ForumTopicOperationError,
  mergeForumTopics,
  moveForumTopic,
  resolveForumTopicRedirect,
  setForumTopicState,
  splitForumTopic,
} from '@/modules/community/forum-topic-operations'
...
export async function GET(
  ...
  try {
    const data = await getForumTopicPosts(payload, {
      siteId: site_id,
      topicId: topic_id,
    ...
    const redirect = await resolveForumTopicRedirect(payload, {
      siteId: site_id,
      topicId: topic_id,
    })
    if (redirect && redirect.targetTopicId !== topic_id)
      return new Response(null, {
        status: 301,
        headers: {
          location: `/api/v1/sites/${site_id}/topics/${redirect.targetTopicId}`,
...
export async function PATCH(
  ...
  const actor = await resolveCommunityActor(payload, request.headers, site_id)
  if (!actor.memberId)
    return json({ error: { code: 'UNAUTHORIZED', message: 'A staff member is required.' } }, 401)
  ...
  try {
    ...
    const data =
      operation === 'move'
        ? await moveForumTopic(payload, {
            siteId: site_id,
            topicId: topic_id,
            targetSpaceId: String(body.targetSpaceId ?? body.target_space_id ?? ''),
            actorId: actor.memberId,
          })
        : operation === 'merge'
          ? await mergeForumTopics(payload, {
              siteId: site_id,
              targetTopicId: topic_id,
              sourceTopicId: String(body.sourceTopicId ?? body.source_topic_id ?? ''),
              actorId: actor.memberId,
            })
          : operation === 'split'
            ? await splitForumTopic(payload, {
                siteId: site_id,
                sourceTopicId: topic_id,
                targetSpaceId: String(body.targetSpaceId ?? body.target_space_id ?? ''),
                title: String(body.title ?? ''),
                postIds: Array.isArray(body.postIds) ? body.postIds.map(String) : [],
                actorId: actor.memberId,
              })
            : await setForumTopicState(payload, {
                siteId: site_id,
                topicId: topic_id,
                actorId: actor.memberId,
    ...
    if (error instanceof ForumTopicOperationError || error instanceof ForumSpaceAccessError)
      return json({ error: { code: error.code, message: error.message } }, error.status)
    return json({ error: { code: 'INTERNAL_ERROR', message: 'An error occurred' } }, 500)
  }
}
