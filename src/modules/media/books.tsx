...
export function BookReader(input: {
  ...
  const released = input.chapters
    .filter((chapter) => isReleasedChapter(chapter))
  ...
  const footnotes = Array.isArray(input.chapter?.footnotes)
    ? (input.chapter.footnotes as RecordDoc[])
    : []
  const bibliography = Array.isArray(input.article?.bibliography)
    ? (input.article.bibliography as RecordDoc[])
    : []
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 py-12 space-y-8">
      <nav aria-label="Breadcrumb" className="text-sm">
        <Link href="/">Home</Link> / <span aria-current="page">{String(input.book.title)}</span>
      </nav>
      <header className="space-y-3">
        <p className="text-sm font-semibold uppercase tracking-wide">Book</p>
        <h1 className="text-4xl font-black">
          {input.chapter ? String(input.chapter.title) : String(input.book.title)}
        </h1>
        {input.chapter ? (
          <p>
            <Link href={String(input.book.canonicalPath)}>Back to table of contents</Link>
          </p>
        ) : typeof input.book.description === 'string' ? (
          <p>{input.book.description}</p>
        ) : null}
      </header>
      <div className="grid gap-8 md:grid-cols-[16rem_1fr]">
          ...
          <ol className="mt-3 space-y-2">
            {released.map((item) => (
              <li key={String(item.id)}>
                <Link
                  aria-current={String(item.id) === currentId ? 'page' : undefined}
                  href={String(item.canonicalPath)}
                >
                  {String(item.title)}
                </Link>
              </li>
        ...
        <article
          ...
          aria-label={input.chapter ? `Chapter: ${String(input.chapter.title)}` : 'Book overview'}
        >
          {input.chapter ? (
            <>
              <div className="prose dark:prose-invert max-w-none whitespace-pre-wrap">
                {content || 'This chapter has no public manuscript yet.'}
              </div>
              {footnotes.length ? (
                <section aria-labelledby="footnotes">
                  ...
                  <ol>
                    {footnotes.map((note, index) => (
                      <li key={String(note.id ?? index)}>
                        {String(note.text ?? note.citation ?? '')}
                      </li>
                    ))}
                  </ol>
                </section>
              ...
              <nav aria-label="Chapter navigation" className="flex justify-between border-t pt-5">
                {previous ? (
                  <Link href={String(previous.canonicalPath)}>
                ...
                {next ? (
                  <Link href={String(next.canonicalPath)}>Next: {String(next.title)}</Link>
                ) : (
                  <span />
                )}
              </nav>
