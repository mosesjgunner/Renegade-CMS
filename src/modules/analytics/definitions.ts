...
export const TELEMETRY_METRIC_DEFINITIONS: Record<string, TelemetryMetricDefinition> = {
  page_views: {
    ...
    formula: 'COUNT(analytics_events WHERE eventType = "page_view" AND consentBasis = "analytics-consent")',
    ...
    uncertaintyDisclosure: 'Represents verified consented visits only. Unconsented traffic is reported separately as missing data.',
  },
  unique_visitors: {
    key: 'unique_visitors',
  ...
  disclosed_campaign_clicks: {
    ...
    grain: 'Raw event (analytics-events)',
    formula: 'COUNT(analytics_events WHERE utm_campaign IS NOT NULL OR ref IS NOT NULL OR channel != "direct")',
    source: 'URL parameters and disclosure-tagged inbound referrals',
    definition: 'Touchpoints arriving via explicit campaign UTM parameters, partner referral codes, or disclosed affiliate links.',
    caveats: [
    ...
    uncertaintyDisclosure: 'Unconsented arrivals via campaigns cannot be tracked or credited without violating privacy.',
  },
  conversions: {
    key: 'conversions',
    ...
    definition: 'Completion of a tracked business objective (newsletter signup, purchase, donation, registration).',
    caveats: [
    ...
    uncertaintyDisclosure: '100% verified when matching canonical orders; labeled as unlinked when visitor identity is absent.',
  },
  conversion_rate: {
    key: 'conversion_rate',
    ...
    uncertaintyDisclosure: 'Standard error and 95% confidence intervals are displayed where applicable.',
  },
  experiment_exposures: {
    ...
    uncertaintyDisclosure: 'Zero exposure telemetry recorded for privacy-shielded or unconsented traffic.',
  },
  practical_effect: {
    ...
    definition: 'Absolute difference in conversion rate between the treatment variant and the control baseline.',
    caveats: [
    ...
    uncertaintyDisclosure: 'Tiny samples (< 100 per variant) are explicitly flagged with insufficient evidence warnings.',
  },
  financial_gross: {
    key: 'financial_gross',
    ...
    definition: 'Total monetary value processed before fees and refunds, strictly segregated by ISO currency.',
    caveats: [
    ...
    uncertaintyDisclosure: 'Distinguishes between settled payments and provider-reported pending attempts.',
  },
}
