...
const limitOf = (value?: number) => Math.min(Math.max(Number.isFinite(value) ? Math.floor(value!) : 30, 1), 100)
...
const cursorFor = (sequence: number) => Buffer.from(JSON.stringify({ sequence })).toString('base64url')

export async function listForumTopics(payload: Payload, input: {
  siteId: string; memberId?: string; spaceId?: string; sort?: string; limit?: number
}) {
  ...
  if (input.spaceId)
    await resolveForumSpaceAccess(payload, { siteId: input.siteId, spaceId: input.spaceId, actor: { memberId: input.memberId } })
  const order = sort === 'creation_date' ? 't.created_at DESC, t.id DESC'
    : sort === 'most_replies' ? 't.reply_count DESC, t.last_post_timestamp DESC NULLS LAST, t.id DESC'
      : 't.last_post_timestamp DESC NULLS LAST, t.id DESC'
  const rows = await executeDbQuery<Record<string, unknown>>(payload, `
    ...
    ORDER BY ${order} LIMIT $5`, [input.siteId, input.memberId ?? null, input.spaceId ?? null, sort, limitOf(input.limit)])
  return rows
}

export async function getForumTopicPosts(payload: Payload, input: {
  siteId: string; topicId: string; memberId?: string; cursor?: string | null; limit?: number; countView?: boolean
}) {
  const topic = (await executeDbQuery<{ id: string; spaceId: string; lastPostSequenceNumber: number }>(payload,
    `SELECT id, space_id AS "spaceId", last_post_sequence_number AS "lastPostSequenceNumber" FROM forum_topics WHERE id = $1 AND site_id = $2`, [input.topicId, input.siteId]))[0]
  if (!topic) throw new ForumBrowsingError('Forum topic not found.', 404, 'TOPIC_NOT_FOUND')
  await resolveForumSpaceAccess(payload, { siteId: input.siteId, spaceId: topic.spaceId, actor: { memberId: input.memberId } })
  if (input.countView !== false)
    await executeDbQuery(payload, 'UPDATE forum_topics SET view_count = view_count + 1 WHERE id = $1', [topic.id])
  ...
  const rows = await executeDbQuery<Record<string, unknown>>(payload, `
    ...
    ORDER BY sequence_number ASC LIMIT $3`, [topic.id, after, limitOf(input.limit) + 1])
...
export async function markForumTopicRead(payload: Payload, input: { siteId: string; topicId: string; memberId: string }) {
  const topic = (await executeDbQuery<{ id: string; spaceId: string; lastPostSequenceNumber: number }>(payload,
    `SELECT id, space_id AS "spaceId", last_post_sequence_number AS "lastPostSequenceNumber" FROM forum_topics WHERE id = $1 AND site_id = $2`, [input.topicId, input.siteId]))[0]
  if (!topic) throw new ForumBrowsingError('Forum topic not found.', 404, 'TOPIC_NOT_FOUND')
  await resolveForumSpaceAccess(payload, { siteId: input.siteId, spaceId: topic.spaceId, actor: { memberId: input.memberId } })
