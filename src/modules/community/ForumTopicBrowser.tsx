...
export type ForumTopicSummary = {
  id: string; title: string; replyCount: number; viewCount: number; unread: boolean
...
export function ForumTopicBrowser({ topics, topicHref = (topic) => `/forums/topics/${topic.id}` }: { topics: ForumTopicSummary[]; topicHref?: (topic: ForumTopicSummary) => string }) {
  ...
  return <section aria-labelledby="forum-topics-heading">
    <div>
      <h1 id="forum-topics-heading">Topics</h1>
      <label htmlFor="forum-topic-sort">Sort topics</label>
      <select id="forum-topic-sort" value={sort} onChange={(event) => changeSort(event.target.value)}>
    ...
    <ul aria-label="Forum topics">
      {topics.map((topic) => <li key={topic.id}>
        <a href={topicHref(topic)}>{topic.unread ? <span aria-label="Unread topic">New: </span> : null}{topic.title}</a>
