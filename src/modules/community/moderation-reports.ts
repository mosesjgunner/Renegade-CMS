...
export function snapshotHash(snapshot: Record<string, unknown>) {
...
async function resolveSnapshot(
  ...
  return JSON.parse(
    stable({ version: 1, targetType: type, targetId: id, siteId, target: row }),
  ) as Record<string, unknown>
}

export async function createCommunityPolicy(
  payload: Payload,
  input: { siteId: string; policyKey: string; policy: Record<string, unknown>; actorId?: string },
) {
  return transaction(payload, async (query) => {
    const key = input.policyKey.trim()
    if (!key || key.length > 128)
      throw new CommunityError('Invalid policy key.', 422, 'INVALID_POLICY_KEY')
    // The lock also covers the first version, where there is no row for FOR UPDATE to lock.
    await query('SELECT pg_advisory_xact_lock(hashtextextended($1, 0))', [
      `policy:${input.siteId}:${key}`,
    ])
    const version = Number(
      (
        await query<{ version: number }>(
          'SELECT COALESCE(MAX(version),0)+1 AS version FROM community_policies WHERE site_id=$1 AND policy_key=$2',
          [input.siteId, key],
        )
      ).rows[0]?.version ?? 1,
    )
    return (
      await query<{ id: string; version: number }>(
        'INSERT INTO community_policies (site_id,policy_key,version,policy_payload,created_by_member_id) VALUES ($1,$2,$3,$4::jsonb,$5) RETURNING id,version',
        [input.siteId, key, version, JSON.stringify(input.policy), input.actorId ?? null],
      )
    ).rows[0]!
  })
}
...
export async function ingestModerationReport(
  payload: Payload,
  input: {
    siteId: string
    reporterId: string
  ...
  if (!moderationTargetTypes.includes(input.targetType))
    throw new CommunityError('Unsupported report target.', 422, 'INVALID_REPORT_TARGET')
  ...
  return transaction(payload, async (query) => {
    ...
    await query('SELECT pg_advisory_xact_lock(hashtextextended($1, 0))', [
      `${input.siteId}:${input.targetType}:${input.targetId}`,
    ])
    let policy: { id: string; version: number } | undefined
    if (input.policyKey)
      policy = (
        await query<{ id: string; version: number }>(
          'SELECT id,version FROM community_policies WHERE site_id=$1 AND policy_key=$2 ORDER BY version DESC LIMIT 1',
          [input.siteId, input.policyKey],
        )
      ).rows[0]
    let parent = (
      await query<{ id: string }>(
        `SELECT id FROM moderation_cases WHERE site_id=$1 AND target_type=$2 AND target_id=$3 AND status='open' AND last_reported_at >= now() - interval '24 hours' ORDER BY last_reported_at DESC LIMIT 1 FOR UPDATE`,
        [input.siteId, input.targetType, input.targetId],
      )
    ).rows[0]
    if (!parent)
      parent = (
        await query<{ id: string }>(
          'INSERT INTO moderation_cases (site_id,target_type,target_id) VALUES ($1,$2,$3) RETURNING id',
          [input.siteId, input.targetType, input.targetId],
        )
      ).rows[0]!
    ...
    const report = (
      await query<{ id: string }>(
        "INSERT INTO community_reports (site_id,reporter_id,target_type,target_id,reason,details,status,parent_case_id,target_snapshot_payload,target_snapshot_hash,policy_id,policy_version) VALUES ($1,$2,$3,$4,$5,$6,'pending',$7,$8::jsonb,$9,$10,$11) RETURNING id",
        [
          input.siteId,
          input.reporterId,
          input.targetType,
...
export function serializeReportForAuthor(record: Record<string, unknown>) {
  return {
    id: String(record.id),
    siteId: String(record.site_id ?? record.siteId),
    targetType: String(record.target_type ?? record.targetType),
    targetId: String(record.target_id ?? record.targetId),
    reason: String(record.reason),
    status: String(record.status),
    parentCaseId: String(record.parent_case_id ?? record.parentCaseId ?? ''),
