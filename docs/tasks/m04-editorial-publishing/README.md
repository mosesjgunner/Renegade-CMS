...
**PUB-03 public publication pass (2026-09-02):** public Pages and Posts now resolve only through an explicitly retained `latestPublishedRevision`; saving a later draft cannot alter that output. Preview tokens are hashed, creator-session-bound, and limited to one hour. Scheduled jobs retain the selected revision, are idempotent, and remain safe when a later draft is saved. Canonical routing renders safe structured Lexical content, while redirects preserve configured query strings, reject loops/hop exhaustion, and record hits. Search uses the same revision resolver.
